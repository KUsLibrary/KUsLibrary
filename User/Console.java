...
public class Console {
	...
	private static final String UserFileString = "D:\\Major_Basic_Project\\src\\User\\UserFile.txt";
	private static final String BookFileString = "D:\\Major_Basic_Project\\src\\User\\BookFile.txt";  // 책 파일 경로!
	...
	@SuppressWarnings("resource")
	public void readBookFile(String file) {		// 프로그램 끝낼 때 파일 새로 저장하기!
		try {
			File userFile = new File(file);
			Scanner fileScan = new Scanner(userFile);
			while(fileScan.hasNextLine()) {
				if (fileScan.nextLine().equals("__BOOK__")) {
					String tempCode = fileScan.nextLine();
					String tempTitle = fileScan.nextLine();
					String tempAuthor = fileScan.nextLine();
					String tempPub = fileScan.nextLine();
					admin.getBookList().add(new Book(tempCode, tempTitle, tempAuthor, tempPub));		// LinkedList에 사용자 정보 한명씩 추가
				}
			}
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
		...
		for (Book book : admin.getBookList()) {
			System.out.println(book.getBookCode() + " " + book.getName() + " " + book.getAuthor() + " " + book.getPublisher());
		}
		...
		if(input.equals("1")) {
			...
			while (true) {
				...
				if (helloUser == null) {
					...
					admin.setUser(uf.run(helloUser, admin.getBookList()));  // 해당 유저의 실행 창 수행
					break;
				}
			}
